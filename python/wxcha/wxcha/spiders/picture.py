...
class PictureSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for i in range(len(elements)):
            ...
            if href is not None:
                content_url = href
                yield response.follow(content_url, callback = self.content, meta = {'type':type, 'group_url':content_url, 'stage':'content'})
    ...
    def content(self, response):
        ...
        for i in range(len(images)):
            ...
            print(concat_str('图片url：', src))
            print(concat_str('分组url：', group_url))
            yield item
        next_url = get_select_first_str(response, "/html/body/div[5]/div[1]/div[5]/div/a[text()='下一页']/@href", None)
        if next_url is not None:
            yield response.follow(next_url, callback = self.content, meta = {'type':type, 'group_url':group_url, 'stage':'content'})